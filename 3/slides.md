...
<div class="flow-step"><span class="flow-num">1</span><span class="flow-label">概念とエコシステム: MCPの基本、3つの機能、コンテキストへの影響、主要なMCP一覧</span></div>
<div class="flow-step"><span class="flow-num">2</span><span class="flow-label">デモ（Playwright MCPによる画面操作）+ ハンズオン（MCPサーバーの統合とコード拡張）</span></div>
<div class="flow-step"><span class="flow-num">3</span><span class="flow-label">セキュリティについて: パス制限、プロセスの権限管理</span></div>
<div class="flow-step"><span class="flow-num">4</span><span class="flow-label">まとめ</span></div>
</div>
...
<!-- visual overflow チェック(Playwright)で確認する。冒頭にクライアント⇄サーバーの関係図を追加している -->
<!-- ignore-layout -->

**Model Context Protocol (MCP)** は、AIエージェントにツール（機能）を提供するための標準プロトコルである。

<div class="flow" style="max-width:660px; margin-left:auto; margin-right:auto;">
<div class="flow-step"><span class="flow-label">AI エージェント<br>（クライアント）</span></div>
<div class="flow-arrow">⇄</div>
<div class="flow-step"><span class="flow-label">MCP サーバー<br>（ツール）</span></div>
</div>

- Anthropic が提唱したオープンスタンダード。LLM単体では学習時点の知識しか持たず社内システムや最新データに直接アクセスできない、という課題を解決する。
- 一度書けば、対応する全クライアント（Antigravity、Claude Code、Codex 等）から利用可能（Plug & Play）。**Tools**、**Resources**、**Prompts**の3つを提供する。
...
MCP サーバーは **Tools**・**Resources**・**Prompts** の3つを提供する。
...
- **用途例**: ファイル編集、データ登録、画面操作まで様々な処理ができる
...
- **用途例**: ログファイルの読み込み／DBのテーブル定義取得／APIから最新データの取得など
...
- **用途例**: コードレビューの定型指示を実行／ボイラープレート生成を指示／バグ解析の指示など
...
.cards .card-title { font-size: 15px; }
</style>

目的に合わせて、多様な外部サービスやツールと接続可能である

<div class="cards" style="--cols:4;">
<div class="card"><span class="card-title">GitHub / GitLab / Backlog</span><span class="card-desc">Issueやタスク・課題の要約、プルリクエストの自動作成</span></div>
<div class="card c-accent"><span class="card-title">Figma</span><span class="card-desc">デザインファイルの読み込みやコンポーネント情報の取得</span></div>
<div class="card c-warm"><span class="card-title">Notion / Obsidian</span><span class="card-desc">ナレッジベースの検索、ページやメモの作成、DB操作</span></div>
<div class="card c-danger"><span class="card-title">Slack / Discord</span><span class="card-desc">チャットの読み書き、チャンネル情報の要約</span></div>
<div class="card"><span class="card-title">Google系 (Gmail / Calendar / Maps)</span><span class="card-desc">メール送信、予定調整、位置検索</span></div>
<div class="card c-accent"><span class="card-title">Playwright</span><span class="card-desc">Webシステムのブラウザ自動操作</span></div>
<div class="card c-warm"><span class="card-title">SQLite / Postgres</span><span class="card-desc">データベースへの接続と自然言語によるデータ抽出</span></div>
</div>

---

## デモ: Playwright MCPを使ってみる
...
1. **アプリ起動**: 
    ```
    cd 3/demo/demo1-playwright-app
    node server.js
    ```
...
1. ディレクトリ移動とパッケージインストール:
    ```
    cd 3/demo/demo2-weather-server/weather-mcp-server
    npm install
    ```
2. エージェントへの統合:
   ローカル設定（`3/demo/demo2-weather-server/.agents/mcp_config.json`）の `/absolute/path/to/...` 部分を絶対パスに書き換える
   ```json
   "weather": {
     "command": "node",
     "args": ["/absolute/path/to/3/demo/demo2-weather-server/weather-mcp-server/index.js"]
   }
   ```
3. 動作確認:
...
## 寄り道: Resources / Prompts も覗いてみる

このサーバーには Tools（`get_weather`）だけでなく、Resources（`weather://` 7日間予報）と Prompts（`suggest-packing-list` 持ち物提案）も実装済み。

**実際に覗いてみる**: `./run_inspector.sh` を実行し、Resources / PromptsタブでInspectorから直接確認できる
...
現在のサーバーは今日の天気しか返さない。
明日の予報も答えられるように、`3/demo/demo2-weather-server/weather-mcp-server/index.js` の `CallToolRequestSchema` を拡張する。
...
**補足**: 期待通り動かない場合、`npx @modelcontextprotocol/inspector node weather-mcp-server/index.js`（または `./run_inspector.sh` で自動実行）でエージェントを介さず直接サーバーの応答を確認できる。
...
1. `3/demo/demo3-response-size/` へ移動し、`.agents/mcp_config.json` の絶対パスを書き換える
...
.compare-imgs { display:flex; gap:10px; align-items:flex-end; margin:0 -30px; }
.compare-imgs img { flex:1 1 0; min-width:0; width:100%; height:auto; }
</style>

<div class="compare-imgs">
<img src="images/demo3-filtered.png">
<img src="images/demo3-raw.png">
</div>
...
| 第2回 | Skills | よく使う手順を再利用する | 済 |
| **第3回** | **MCP** | **AI のできることを増やす** | **完了** |
| 第4回 | Subagents | 重い作業を別の頭脳に任せる | 次回 |

- **次回予告: 第4回 Subagents**
  - 重い調査や専門作業を独立したコンテキストで動く子エージェントに委譲する仕組みを学ぶ。
  - Subagentsの並列実行によるスケールアウトを体験する。
