...
完成済みのシンプルなMCPサーバーをエージェントに統合して動作を確認したのち、実際にコードを書き換えて機能を拡張します。
APIキー不要のパブリックAPI（PokéAPI）をラップしたサーバーを通じて、MCPの仕組みを実践的に理解します。

### ステップ 1: クローンと動作確認 (10 分)

用意されたリポジトリを利用し、エージェントへの統合を体験します。
...
1. お使いのAIエージェント（agy等）にMCP設定を追加し、サーバーを登録します。
2. エージェントに「リザードンのタイプと体重を教えて」と質問し、API経由で情報を取得して回答することを確認します。

---

### ステップ 2: コード構造の理解 (5 分)

`index.js` の主要な構成要素（約40行）を確認します。

- **ツールの定義**: `ListToolsRequestSchema` で `get_pokemon_info` ツールと引数の型（JSON Schema）を定義します。
- **処理の実装**: `CallToolRequestSchema` 内で外部API（PokéAPI）を叩き、取得したJSONから必要な文字列を抽出して返します。

---

### ステップ 3: 機能拡張に挑戦 (10 分)

現在のサーバーは「名前」「タイプ」「身長」「体重」しか返しません。
エージェントが**「ポケモンの特性（abilities）」**も答えられるように、コードを拡張してみましょう。

**ヒント**:
PokeAPIのレスポンスには `data.abilities` という配列が含まれています。これを `.map()` 等で展開し、返す文字列（`text`変数）に追加します。修正後はサーバープロセスを再起動して動作を確認してください。
