...
単発の測定では分割版の方が多くなることがあるが、実開発では分割設計が不可欠である。

- **複数ターンでのトークン累積防止**
  - 単一ファイルの場合、毎ターンの対話で巨大なルールが重複ロードされ、会話が長引くほど消費が累積する。
- **ノイズの排除による精度向上**
  - タスクに無関係なルール（設計時のテストやGitルールなど）を遮断することで、AIの注意力を散漫にさせず規約遵守率を保つ。
- **大規模開発へのスケールアウト**
  - プロジェクトの成長に伴いルールが増大した際、すべての常時ロードは不可能。必要な情報のみ動的ロードする設計が必須となる。
...
| # | テーマ | 概要 | 状態 |
|---|---|---|:---:|
| 第1回 | AI Agent の全体像 | エージェントとは何かを掴む | 済 |
| **第2回** | **コンテキスト管理 + プロンプト設計** | **AI に何を・どう伝えるか** | **完了** |
| 第3回 | Skills | よく使う手順を再利用する | 次回 |
| 第4回 | MCP | AI のできることを増やす | |
| 第5回 | Subagents | 重い作業を別の頭脳に任せる | |
