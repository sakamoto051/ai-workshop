...
**実行手順**
1. **アプリ起動**: `cd s4/demo/demo1-playwright-app && npx -y http-server -p 3000` を実行し動作確認
...
1. ディレクトリ移動とパッケージインストール:
   `cd s4/demo/demo2-pokemon-server/pokemon-mcp-server && npm install`
2. エージェントへの統合:
   ローカル設定（`s4/demo/demo2-pokemon-server/.agents/mcp_config.json`）の `/absolute/path/to/...` 部分を絶対パスに書き換える
   ```json
   "pokemon": {
     "command": "node",
     "args": ["/absolute/path/to/s4/demo/demo2-pokemon-server/pokemon-mcp-server/index.js"]
   }
   ```
3. 動作確認:
...
ポケモンの特性（abilities）も答えられるように、`s4/demo/demo2-pokemon-server/pokemon-mcp-server/index.js` の `CallToolRequestSchema` を拡張する。
...
|---|---|---|---|
| **Before**（生データをそのまま返す） | 技・ゲームバージョン別データ等を含む全JSON | 約 288,000 文字 | **約 72,000 トークン**（英語JSON基準の目安） |
| **After**（本ハンズオンの実装） | 名前・タイプ・身長・体重に絞り込んだテキスト | 42 文字 | 数十トークン程度（※） |

絞り込みの有無で約 6,900 倍の差（文字数ベース）。→ サーバー側で本当に必要な項目だけを抽出・整形して返すことが、コンテキスト管理の実務である。

※日本語混じりのテキストは英語中心のJSONより1文字あたりのトークン効率が悪いため、Before と同じ換算比（約4文字/トークン）は適用していない。
