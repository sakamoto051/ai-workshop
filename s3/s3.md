...
1. **アプリ起動**: 
    ```
    cd s3/demo/demo1-playwright-app
    node server.js
    ```
...
1. ディレクトリ移動とパッケージインストール:
    ```
    cd s3/demo/demo2-weather-server/weather-mcp-server
    npm install
    ```
2. エージェントへの統合:
   ローカル設定（`s3/demo/demo2-weather-server/.agents/mcp_config.json`）の `/absolute/path/to/...` 部分を絶対パスに書き換える
   ```json
   "weather": {
     "command": "node",
     "args": ["/absolute/path/to/s3/demo/demo2-weather-server/weather-mcp-server/index.js"]
   }
   ```
3. 動作確認:
...
現在のサーバーは今日の天気しか返さない。
明日の予報も答えられるように、`s3/demo/demo2-weather-server/weather-mcp-server/index.js` の `CallToolRequestSchema` を拡張する。
...
1. `s3/demo/demo3-response-size/` へ移動し、`.agents/mcp_config.json` の絶対パスを書き換える
